$(document).ready(function() {
  ...
  const destination_image_p1 =
    "https://res.cloudinary.com/lux-group/image/upload/f_auto,fl_progressive,q_auto:eco,c_fill,g_auto,ar_16:9/";
  const destination_image_p2 = [];
  // Video
  const destination_video = [];
  const destination_video_url =
    "https://res.cloudinary.com/lux-group/video/upload/vc_auto,q_70/";
  const destination_video_poster = [];
  const destination_video_poster_url =
    "https://res.cloudinary.com/lux-group/video/upload/so_auto/";
  // Reviews
  ...
  const api = fetch("https://api.luxgroup.com/api/public-offers")
    ...
    .then(function(myJson) {
      ...
      for (let i = 0; i < destinations_array; i++) {
        ...
        let details_offer = `${details_link_p1}${details_link_p2[i]}/${
          details_link_p3[i]
        }`;
        details_link.push(details_offer);
        // Images
        destination_image_p2.push(
          myJson.result[i].images[0].id_cloudinary_external
        );
        let destination_image = `${destination_image_p1}${
          destination_image_p2[i]
        }`;
        destination_images.push(destination_image);
        // Video
        let destination_video_link = `${destination_video_url}${
          myJson.result[i].video_cloudinary_id
        }.mp4`;
        destination_video.push(destination_video_link);
        let destination_video_poster_link = `${destination_video_poster_url}${
          myJson.result[i].video_cloudinary_id
        }.jpg`;
  ...
  function initMap() {
    ...
    for (let i = 0; i < destination_coordinates.length; i++) {
      let marker = new google.maps.Marker({
        position: {
          lat: destination_coordinates[i][0],
          lng: destination_coordinates[i][1]
        },
        map: map
      });
      ...
      marker.addListener("click", function() {
        ...
        markerOffer(
          ...
          destinationDetailsButton,
          destinationImage,
          destinationVideo,
          destinationVideoPoster,
          destinationReview,
          destinationReviewSource
        );
      });
    }
    ...
    markerOffer(
      ...
      destination_price[0],
      destination_accommodation[0],
      details_link[0],
      destination_images[0],
      destination_video[0],
      destination_video_poster[0],
      destination_review[0],
      destination_review_source[0]
    );
  }
  ...
  const changeBackground = () => {
    setTimeout(function() {
      $(".hero").css(
        "background-image",
        `url(${destination_images[backgroundCounter]})`
      );
